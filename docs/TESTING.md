...
| `worker-ipc/test-scan-edge-cases.js` | worker-ipc | Disc scans (backup planning, the Add missing files source scan) never follow a link: each link is planned as the one small Windows shortcut it is burned as, created in the temp folder when its disc is sent and read back by Windows as pointing where the link points - nothing behind a link gets into the plan; an unreadable folder is named in one "Some items were left out" warning where a wizard asks for that, and fails a disc read and a sync comparison; every too-large file is reported, not just the first, without a premature warning; a whole drive (a temporary `SUBST` drive) works as the folder. |
| `worker-ipc/test-temp-dir-and-imgburn.js` | worker-ipc | The temp/cache folder's ownership rules (a moved app folder's temp folder is adopted, a folder with foreign content refused), clearing the app's own content (split parts and link shortcuts), the per-file lists the clean-up steps return, and ImgBurn launch reporting (a missing executable shows one error, the `.ibb` path is passed quoted, a non-zero exit shows nothing). Uses scratch folders only. |
| `worker-ipc/test-sync-and-cumulative-rules.js` | worker-ipc | Both folder features against their rules: 20 random folder pairs each (Cumulative adds missing and updated files and never deletes or changes anything else; Synchronize directories leaves the target identical to the source); a read-only or hidden earlier copy is replaced; sync's 2-second modified-time allowance; links (junctions) are single entries that are copied or deleted as links and never followed, with a folder outside both proven untouched; folders inside each other are refused; the same folder twice is simply up to date; a name that is a file on one side and a folder on the other is replaced by Sync and kept - renamed "(old folder)"/"(old file)" - by Cumulative. The check after a sync (compare-folders) confirms every sync above and reports each kind of difference, one line each. Sync's delete step never deletes through a link: a target folder the copy step replaced with a link is left alone, while the rest of the list is still deleted. A copy that fails part way (part of the source locked by another program) leaves the target's earlier copy as it was, with no temporary file behind. |
