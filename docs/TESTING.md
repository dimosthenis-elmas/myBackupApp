...
| `ui/test-wizard-error-dialogs.js` | UI | Folders inside each other are refused by both Cumulative backup and Synchronize directories with a readable message naming both (and nothing changes); a source folder Windows refuses to list is named by full path in a "Some items were left out" warning while the rest is still compared. After a sync that leaves a difference (a case-only rename), the check shows it in a "differences found" list. |
