...
- `optical-media/` (`OpticalMediaTestKit.psm1`) - makes a `.iso` file look exactly like a real inserted optical
  ...
  drive. `ui/iso-disc.js` never mounts one as `D:` when that letter is free (it holds `D:` with a temporary `SUBST`
  while mounting): the app writes every disc's paths with a fixed `D:\`, whatever letter the disc really has, so a
  disc under another letter - as on most real machines - is what has to be exercised.
...
| `worker-ipc/test-incremental-backup.js` | worker-ipc | Cumulative backup: a first sync copies everything; a second sync after changes reports and copies *exactly* the changed files (not the whole tree again), proven via mtime, not just re-hashing. Plus edge cases: `$&` in the source folder name, a newer copy in the backup and a backup-only file left alone, a folder renamed in letter case only, an unreadable entry skipped and reported, a file vanishing before the copy, a read-only earlier copy replaced. |
| `worker-ipc/test-sync-dirs.js` | worker-ipc | Synchronize directories: copies changes AND deletes files that only exist in the target. The one script here that can genuinely delete real files - see its own safety writeup. Also: a target copy that is newer, or has the same size and date but different bytes, is overwritten, never deleted; letter-case-only renames (with and without an edit) on NTFS, which end up with the source's spelling; and a case-sensitive folder, where the two spellings are different files. |
| `worker-ipc/test-scan-edge-cases.js` | worker-ipc | Disc scans (backup planning, the Add missing files source scan) never follow a link: each link is left out - nothing behind it gets into the plan - and written to logs.txt with where it points, never to the "Some items were left out" warning, and counted in the response (for the wizard's dialog); an unreadable folder is named in that one warning where a wizard asks for that, and fails a disc read and a sync comparison; every too-large file is reported, not just the first, without a premature warning; a whole drive (a temporary `SUBST` drive) works as the folder. |
| `worker-ipc/test-temp-dir-and-imgburn.js` | worker-ipc | The temp/cache folder's ownership rules (a moved app folder's temp folder is adopted, a folder with foreign content refused), clearing the app's own content (split parts), the per-file lists the clean-up steps return, and ImgBurn launch reporting (a missing executable shows one error, the `.ibb` path is passed quoted, a non-zero exit shows nothing). Uses scratch folders only. |
| `worker-ipc/test-sync-and-cumulative-rules.js` | worker-ipc | Both folder features against their rules: 20 random folder pairs each (Cumulative adds missing and updated files and never deletes or changes anything else; Synchronize directories leaves the target identical to the source); a read-only or hidden earlier copy is replaced; sync's 2-second modified-time allowance; links (junctions) are never followed or copied - a source link is left out, written to logs.txt and counted in the comparison's response (no warning), a target link is replaced or deleted as the link itself - with a folder outside both proven untouched; folders inside each other are refused, and so is a chosen folder that is a link or inside one, also recovery's (a SUBST drive is not); the same folder twice is simply up to date; a name that is a file on one side and a folder on the other is replaced by Sync and kept - renamed "(old folder)"/"(old file)" - by Cumulative. The check after a sync (compare-folders) confirms every sync above and reports each kind of difference, one line each. Sync's delete step never deletes through a link: a target folder that became a link after the list was made is left alone, while the rest of the list is still deleted. A copy that fails part way (part of the source locked by another program) leaves the target's earlier copy as it was, with no temporary file behind. |
| `worker-ipc/test-long-names-on-disc.js` | worker-ipc | Names over 127 characters (a file, a folder, an empty folder, one with an emoji where it would be cut, a split file's pieces): the `.ibb` project names each as it will be on the disc - shortened, a piece keeping its `.part.NNN` and all pieces one start, a name of exactly 127 kept - reads it from the untouched original, and adds the disc's list of original names. The **real ImgBurn** builds that project without changing a name, and the mounted image holds exactly those names and contents; recovery's copy puts every original name back from the disc's own list. Refused: a list that would take a user file's name, and recovery paths leading outside their folders. The source files are unchanged. Then a real 700 MB file whose 120-character name fits but whose pieces' names do not, the whole way: split by 7-Zip, both discs built by ImgBurn, each recovered under the original piece names, and rejoined into the file under its own name, byte for byte. Clearing the temp folder removes the lists and the pieces. Uses scratch folders only; needs ImgBurn. |
...
| `ui/test-recover-single-disc.js` | UI | The full "Recover data from optical media backup" wizard, one disc, clicked through on screen. A recovery folder that is not empty is refused at "Next", and one that gets a file after "Next" is refused again right before copying ("Choose another folder"); both are left exactly as they were. |
| `ui/test-incremental-backup.js` | UI | The full "Cumulative backup" wizard, clicked through on screen. |
| `ui/test-sync-dirs.js` | UI | The full "Synchronize directories" wizard, clicked through on screen - the one UI script that can genuinely delete real files. Checks the success dialog states the number of files and their total size, and that the Cancel button is gone afterwards. |
...
| `ui/test-backup-to-optical-media-sha256.js` | UI | The SHA-256 integrity-checksum feature's backup-side hashing (mandatory, not a toggle), through the real "Backup to optical media" wizard: reads the real saved metadata JSON back and independently re-hashes every source file to confirm the recorded `sha256` is actually correct, and that no directory entry carries one. |
...
| `ui/test-sync-dirs-cancel.js` | UI | Cancel in Synchronize directories at each moment it can be pressed - on the warning, straight after confirming (before the commit has started), and while files are being copied: it always stops cleanly, shows no error, and nothing is deleted after it. |
| `ui/test-wizard-error-dialogs.js` | UI | Folders inside each other are refused by both Cumulative backup and Synchronize directories with a readable message naming both (and nothing changes); a source folder Windows refuses to list is named by full path in a "Some items were left out" warning while the rest is still compared. A sync whose only difference is a rename in capital letters previews and makes the rename; a difference the check after a sync finds is shown in a "differences found" list. |
| `ui/test-recover-copy-failure.js` | UI | A recovery copy that fails (the recovery folder made unwritable) shows the wizard's own "Try this disc again / Cancel recovery" dialog; trying again once the folder is writable recovers every file. |
| `ui/test-install-path-special-characters.js` | UI | A copy of the built app in a folder named with `#` and `%` starts and shows its main menu, loads nothing from the internet, and uses its own bundled fonts and icons. |
| `ui/test-long-names-on-disc.js` | UI | Names over 127 characters and paths over 259, end to end: "Backup to optical media" lists every long name by full path, and "Cancel" stops before anything is planned; "Continue" then lists the long path, and the disc's metadata JSON records each shortened file with its original path, plus the disc's list of original names. The **real ImgBurn** builds the disc from the app's project. Recovery from the JSON shows the original names, lists the recovered paths that are too long, asks again after "Choose another folder", and recovers every original name, SHA-256 verified; recovery from the disc alone shows and recovers them too. "Verify integrity" recognizes the disc and verifies every file on it, the list of original names included. "Add missing files" - reading the disc, and with the JSON - finds only a newly added long-named file missing; with the JSON it lists it and records its original path. The source files are never changed. Needs ImgBurn. |
| `ui/test-long-names-split-file.js` | UI | A real 700 MB file whose 120-character name fits on a disc but whose pieces' names do not, through the wizards: "Backup to optical media" splits it and lists the file itself (once) as too long; both discs are built by the **real ImgBurn** without a name changed; "Recover data" from the JSON shows the pieces under their original names, takes both discs in turn, offers to rejoin the file under its original name, and the rejoined file is byte for byte the original. Needs ImgBurn and about 3 GB free. |
...
disc," was added alongside the SHA-256 integrity-checksum feature (a mandatory per-file hash recorded at backup
time, checked again on recovery or via that standalone wizard; older cold storage JSONs predating this feature
simply have no recorded hash, reported as "no integrity data available" rather than a failure). Not yet built:
the "add missing files" screen's
